...
export default function AuthButton() {
  ...
  // Identifies if the web context is explicitly constrained by Apple's tracking prevention engines.
  const isWebKitBrowser = isAppleSystem && typeof window !== 'undefined' && !window.hasOwnProperty('android');
  ...
  // If Safari is intercepted, we pass authorization states via strict window assignment links 
  // straight through Google Identity endpoints to completely sidestep ITP cookie blocks.
  const handleWebKitGoogleRedirect = () => {
    if (typeof window === 'undefined') return;
    const targetClientId = process.env.NEXT_PUBLIC_GOOGLE_CLIENT_ID || '';
    // Maps standard OAuth callback parameters matching your backend routing structures
    const oauthUrl = `https://accounts.google.com/o/oauth2/v2/auth?client_id=${targetClientId}&redirect_uri=https://glide-sports.onrender.com/api/auth/google/callback&response_type=code&scope=profile%20email&state=${encodeURIComponent(window.location.origin)}`;
          ...
          <div className="w-full max-w-[400px] bg-[#0F1117] rounded-3xl p-8 border border-white/10 shadow-2xl relative">
            ...
            <div className="space-y-4 w-full flex flex-col items-center">
              ...
              {isNative ? (
                ...
                // Replaces the iframe pop-up engine with a clean top-level location redirect on iOS/macOS Safari.
                <button 
                  onClick={handleWebKitGoogleRedirect} 
                  className="flex items-center justify-center w-[320px] h-10 px-4 rounded-full border border-gray-700 bg-transparent hover:bg-white/5 text-white text-sm font-medium transition-colors active:scale-[0.98]"
                >
                  <svg className="w-4 h-4 mr-3" viewBox="0 0 24 24">
                    <path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" fill="#4285F4" />
                ...
                /* Web Google wrapper matching standard 40px layout for normal non-Safari channels */
